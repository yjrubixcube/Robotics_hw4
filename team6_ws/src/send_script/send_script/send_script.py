...
def find_blocks(image):
    ...
    bin_img_color = cv2.cvtColor(bin_img, cv2.COLOR_GRAY2BGR)
    ...
    for cnt in contours:
        ...
        cv2.line(bin_img_color,(cx,cy),(int(cx+100*cos(angle*np.pi/180)),int(cy+100*(angle*np.pi/180))),(0,0,255),5)
    cv2.imwrite("aaa.png",bin_img_color)
    return block_centers

CAMERA_MTX=np.array([[1.32e+03, 0.00000000e+00, 6.48905477e+02],\
                    [0.00000000e+00, 1.32e+03, 4.60660056e+02],\
...
CAMERA_OFFSET=np.array((-98,2,45))# the coordinate is the gripper coordinate
...
def pickup_blocks(block_centers):
    ...
    for block_center in block_centers:
        ...
        print((block_center[2]))
        ## move upwards by 100
        target_block_upward=f"{block_pos[0]}, {block_pos[1]}, {GRIP_HEIGHT+50},-180,0, {-(block_center[2])-45}"
        move_to_target_block_upward="PTP(\"CPP\","+target_block_upward+",100,200,0,false)"
        print(move_to_target_block_upward)
        send_script(move_to_target_block_upward)
        ## move down to grip
        target_block=f"{block_pos[0]}, {block_pos[1]}, {GRIP_HEIGHT},-180, 0, {-(block_center[2])-45}"
        move_to_target_block=f"PTP(\"CPP\",{target_block},100,200,0,false)"
        send_script(move_to_target_block)
        grip(1.0)
        send_script(move_to_target_block_upward)
        ## move upwards by 100
        stack_pos_upward=f"120,450,{50+GRIP_HEIGHT+target_height},-180,0,90"
        move_upward = "PTP(\"CPP\","+stack_pos_upward+",100,200,0,false)"
        send_script(move_upward)
        ## move down to place
        stack_pos=f"120,450,{GRIP_HEIGHT+target_height},-180,0,90"
...
def main(args=None):
    ...
    #photo_pos = "120,450, 100, -180.00, 0.0, 45"
